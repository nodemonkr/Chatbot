...
const Chatbot = (props) => {
  ...
  const chatBotCall = () => {
    API.post('/chatbot', { question: message })
      .then((res) => {
        setContents([
          ...contents,
          { name: 'haibot', message: res.data.response },
        ]);
      })
      .catch((err) => {
      ...
      <Hidden xsDown>
        <Drawer
          ...
          <div className={classes.drawerContainer}>
            <List>
              {['직업별 질문 챗봇', '기업별 질문 챗봇', '성격별 질문 챗봇'].map(
                (text, index) => (
        ...
        <div className={styles.chatbotRight}>
          ...
          <Typography
            className={classNames({
              [styles.chatbotFont]: true,
              [styles.chatbotName]: true,
            })}>
            # 직업별 질문 챗봇
          </Typography>
          {/* 대화 내용 */}
                ...
                <ul className={styles.chatbotTalk}>
                  ...
                  {contents.map((item) => {
                    if (item.name == 'haibot') {
                      ...
                      return (
                        <li className={styles.getTalk}>
                          <span className={styles.chatbotImage}>
                            <img
                              src='images/임시chaticon.png'
                              width='30'
                              height='30'
                            />
                          </span>
                          <div claaName={styles.talkContainer}>
                            <strong className={styles.talkChatbotName}>
                              하이밋
                            </strong>
                            <br />
                            ...
                            <span className={styles.getTalkAbout}>
                              ...
                              <div>
                                <p className={styles.talkTime}>11시 40분</p>
                              </div>
                            </span>
                          </div>
                        </li>
